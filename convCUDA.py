...
class Conv3x3:
  # A Convolution layer using 3x3 filters.
    ...
    def forward_batch(self, inputs):
        # inputs shape: (N, H, W)
        self.last_input_batch = inputs  # store batch input
        N, H, W = inputs.shape
        # We'll compute 'valid' convolution for each filter over the batch
        out_list = []
        for f in range(self.num_filters):
            # correlate applies over each image in the batch in parallel
            out_f = correlate(inputs, self.filters[f], mode='valid')
            out_list.append(out_f)
        # Stack so that output has shape (N, H-2, W-2, num_filters)
